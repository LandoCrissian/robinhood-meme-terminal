...
assert.equal(fees.rows.length, 44); assert.equal(fees.feeAdmissionChanged, false);
for (const row of fees.rows) {
  ...
  if (row.zeroXReturnedAmount !== null) assert.equal(row.zeroXReturnedAmount, ((numerator + 5000n) / 10000n).toString());
}
