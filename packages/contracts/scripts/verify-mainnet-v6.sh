...
REGISTRY_GOVERNANCE="0x13C0A930516FB6bF0d467B38605d9D2a9c4C6953"
VERSION_REGISTRY="0x4b8b222B5CAa7066c02A54E51eC1a674ADf5b3A1"
...
require_code "V5 registry governance" "$REGISTRY_GOVERNANCE"
require_code "version registry" "$VERSION_REGISTRY"
...
expect_call "registry-governance operator signer" "$REGISTRY_GOVERNANCE" "isSigner(address)(bool)" "true" "$OPERATOR"
expect_call "registry-governance signer count" "$REGISTRY_GOVERNANCE" "signerCount()(uint256)" "1"
expect_call "registry-governance threshold" "$REGISTRY_GOVERNANCE" "threshold()(uint256)" "1"
expect_call "registry-governance delay" "$REGISTRY_GOVERNANCE" "executionDelay()(uint256)" "$GOVERNANCE_DELAY"
expect_call "pre-release registry-governance proposal count" "$REGISTRY_GOVERNANCE" "transactionCount()(uint256)" "0"
...
expect_call "registry governance" "$VERSION_REGISTRY" "governance()(address)" "$REGISTRY_GOVERNANCE"
expect_call "registry activation delay" "$VERSION_REGISTRY" "activationDelay()(uint256)" "$REGISTRY_ACTIVATION_DELAY"
expect_call "pre-activation factory" "$VERSION_REGISTRY" "activeFactory()(address)" "$LEGACY_FACTORY"
expect_call "pre-activation version" "$VERSION_REGISTRY" "activeVersion()(bytes32)" "$V5_VERSION"
expect_call "pending factory" "$VERSION_REGISTRY" "pendingFactory()(address)" "$ZERO_ADDRESS"
expect_call "pending version" "$VERSION_REGISTRY" "pendingVersion()(bytes32)" "$ZERO_BYTES32"
expect_call "pending activation time" "$VERSION_REGISTRY" "pendingActivationTime()(uint64)" "0"

echo "Checking the seven operator-supplied V6 contracts"
...
expect_call "policy treasury" "$V6_POLICY_REGISTRY_ADDRESS" "canonicalProtocolTreasury()(address)" "$OPERATOR"
...
expect_call "factory version registry" "$V6_FACTORY_ADDRESS" "factoryRegistry()(address)" "$VERSION_REGISTRY"
...
echo "All onchain checks passed. Starting exact Blockscout source verification for thirteen contracts."

REGISTRY_GOVERNANCE_ARGS="$(cast abi-encode 'constructor(address,uint256)' "$OPERATOR" "$GOVERNANCE_DELAY")"
...
REGISTRY_ARGS="$(cast abi-encode 'constructor(address,uint256,address,bytes32)' "$REGISTRY_GOVERNANCE" "$REGISTRY_ACTIVATION_DELAY" "$LEGACY_FACTORY" "$V5_VERSION")"
...
POLICY_ARGS="$(cast abi-encode 'constructor(address,address,uint64,address,address,address)' "$V6_GOVERNANCE_ADDRESS" "$OPERATOR" "$GOVERNANCE_DELAY" "$OPERATOR" "$V6_MARKET_IMPLEMENTATION_ADDRESS" "$V6_ADAPTER_ADDRESS")"
FACTORY_ARGS="$(cast abi-encode 'constructor(address,address,address,uint256,uint256,address,address,address)' "$V6_LAUNCH_GATE_ADDRESS" "$V6_POLICY_REGISTRY_ADDRESS" "$VERSION_REGISTRY" "$INITIAL_VIRTUAL_ETH_RESERVE" "$INITIAL_VIRTUAL_TOKEN_RESERVE" "$LEGACY_FACTORY" "$OFFICIAL_LEGACY_RMT_TOKEN" "$OPERATOR")"
...
verify_contract "V5 registry governance" "$REGISTRY_GOVERNANCE" "src/ExpandableGovernance.sol:ExpandableGovernance" "$REGISTRY_GOVERNANCE_ARGS"
verify_contract "V6 governance" "$V6_GOVERNANCE_ADDRESS" "src/RMTV6Governance.sol:RMTV6Governance" "$V6_GOVERNANCE_ARGS"
verify_contract "V5 version registry" "$VERSION_REGISTRY" "src/VersionedFactoryRegistry.sol:VersionedFactoryRegistry" "$REGISTRY_ARGS"
...
echo "V6 source verification passed for all thirteen contracts. No blockchain transaction was broadcast and V6 remains inactive and paused."
