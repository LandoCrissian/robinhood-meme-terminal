...
import {VerifySyntheticCommodityEvidenceRegistryV0} from "../script/VerifySyntheticCommodityEvidenceRegistryV0.s.sol";
...
contract RMTCommodityEvidenceRegistryV0PostflightTest {
    ...
    function setUp() public {
        ...
        registry.registerParty(
            ISSUER_PARTY_ID, vm.addr(ISSUER_KEY), registry.ROLE_ISSUER_BITMAP(), validFrom, validUntil
        );
        registry.registerParty(
            CUSTODIAN_PARTY_ID, vm.addr(CUSTODIAN_KEY), registry.ROLE_CUSTODIAN_BITMAP(), validFrom, validUntil
        );
        registry.registerParty(
            ATTESTOR_PARTY_ID, vm.addr(ATTESTOR_KEY), registry.ROLE_ATTESTOR_BITMAP(), validFrom, validUntil
        );
    ...
    function testPostflightVerifiesExactSyntheticConfiguration() public {
        bytes32 domain =
            verifier.run(address(registry), address(this), address(registry).codehash, validFrom, validUntil);
        ...
        (bool success,) = address(verifier)
            .call(
                abi.encodeWithSelector(
                    verifier.run.selector, address(registry), address(this), bytes32(uint256(1)), validFrom, validUntil
                )
            );
    ...
    function testPostflightRejectsUnconfiguredRegistry() public {
        RMTCommodityEvidenceRegistryV0 unconfigured = new RMTCommodityEvidenceRegistryV0(address(this));
        (bool success,) = address(verifier)
            .call(
                abi.encodeWithSelector(
                    verifier.run.selector,
