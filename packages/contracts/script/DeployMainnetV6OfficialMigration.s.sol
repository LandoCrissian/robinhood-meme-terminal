...
///      cannot broadcast to mainnet, activate V6, launch the official token, or reopen launches.
contract DeployMainnetV6OfficialMigration {
    ...
    function run() external {
        ...
        RMTLaunchGate launchGate = new RMTLaunchGate(
            address(governance), Config.INITIAL_GUARDIAN, Config.LAUNCH_UNPAUSE_DELAY, address(bootstrapController)
        );
        ...
        if (
            address(hook) != expectedHook || (uint160(address(hook)) & uint160(0x3fff)) != flags
                ...
                || bootstrapController.governance() != address(governance)
                || bootstrapController.CHAIN_ID() != Config.CHAIN_ID
                || bootstrapController.OPERATOR() != Config.DEVELOPER_OPERATOR
                || bootstrapController.BOOTSTRAP_WINDOW() != Config.BOOTSTRAP_WINDOW
                || bootstrapController.state() != RMTV6BootstrapController.BootstrapState.Unbound
                || !bootstrapController.bootstrapAvailable()
                || address(bootstrapController.foundationVerifier()).code.length == 0
                || address(bootstrapController.smokeVerifier()).code.length == 0
                || bootstrapController.foundationVerifier().controller() != address(bootstrapController)
                || bootstrapController.smokeVerifier().controller() != address(bootstrapController)
                || launchGate.governance() != address(governance) || launchGate.guardian() != Config.INITIAL_GUARDIAN
                || launchGate.bootstrapController() != address(bootstrapController) || launchGate.bootstrapConsumed()
                || !launchGate.launchesPaused() || launchGate.unpauseDelay() != Config.LAUNCH_UNPAUSE_DELAY
                || launchGate.unpauseExecutableAt() != 0 || launchGate.unpauseExpiresAt() != 0
                || launchGate.unpauseConfigurationEpoch() != 0 || policyRegistry.governance() != address(governance)
                ...
                || policyRegistry.defaultPolicyId() != Config.DEFAULT_POLICY_ID
                || policyRegistry.policyHash(Config.SIMPLE_FAIR_V1_POLICY_ID) == bytes32(0)
                || policyRegistry.policyHash(Config.SIMPLE_OPEN_V1_POLICY_ID) == bytes32(0)
                || factory.protocolVersion() != 6 || address(factory.launchGate()) != address(launchGate)
                ...
                || versionRegistry.bootstrapController() != address(bootstrapController)
                || versionRegistry.bootstrapConsumed()
                ...
                || versionRegistry.pendingActivationTime() != 0 || versionRegistry.pendingExpirationTime() != 0
                || versionRegistry.pendingConfigurationEpoch() != 0
        ...
        emit V6FoundationDeployed(
            ...
            address(versionRegistry),
            address(bootstrapController),
            address(bootstrapController.foundationVerifier()),
            address(bootstrapController.smokeVerifier()),
            address(adapter),
            address(hook),
            address(marketImplementation)
        );
