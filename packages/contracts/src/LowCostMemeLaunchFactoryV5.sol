...
interface IPurposeRewardsControllerV5 {
    function registerVault(address vault, address token, bytes32 purpose) external;
}

interface ILegacyProtectedIdentityFactory {
...
/// @notice Mainnet V5 factory with fixed-recipient settlement and inherited V4 identity protection.
contract LowCostMemeLaunchFactoryV5 {
        ...
        returns (address token, address market, address rewardVault)
    {
        ...
        IPurposeRewardsControllerV5(rewardsController).registerVault(community, token, COMMUNITY_PURPOSE);
        IPurposeRewardsControllerV5(rewardsController).registerVault(trader, token, TRADER_PURPOSE);
    ...
    function isNameUsed(string calldata name) external view returns (bool) {
        return usedNameHashes[_canonicalName(name)]
            || ILegacyProtectedIdentityFactory(legacyIdentityFactory).isNameUsed(name);
    }

    function isSymbolUsed(string calldata symbol) external view returns (bool) {
        return usedSymbolHashes[_canonicalSymbol(symbol)]
            || ILegacyProtectedIdentityFactory(legacyIdentityFactory).isSymbolUsed(symbol);
    }
        ...
        returns (bytes32 nameHash, bytes32 symbolHash)
    {
        ...
        ILegacyProtectedIdentityFactory legacy = ILegacyProtectedIdentityFactory(legacyIdentityFactory);
