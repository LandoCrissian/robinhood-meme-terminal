...
contract LowCostMemeLaunchFactory {
    ...
    function launch(
        ...
        uint256 totalBps;
        for (uint256 i; i < rewardBps.length; ++i) totalBps += rewardBps[i];
        ...
        CloneFixedSupplyMemeToken(token).initialize(
            name, symbol, TOKEN_SUPPLY, msg.sender, address(this), metadataURI
        );
        ...
        CloneBondingCurveMarket(payable(market)).initialize(
            token,
            payable(rewardVault),
            graduationAdapter,
            poolId,
        ...
        _launches.push(Launch(token, market, rewardVault, poolId, msg.sender, uint64(block.timestamp)));
        emit TokenLaunched(
            launchId, token, msg.sender, market, rewardVault, poolId, name, symbol, metadataURI, rewardBps
        );
    }
