...
/// @notice Immutable accounting router for RMT's share of launch-market fees.
/// @dev There is no owner, arbitrary call, recipient update, or withdrawal bypass.
///      Each recipient should be a purpose-specific multisig or immutable vault.
contract ProtocolRevenueRouter {
    ...
    constructor(address[5] memory recipients_) {
        for (uint256 i; i < recipients_.length; ++i) {
            ...
            for (uint256 j; j < i; ++j) {
                if (recipients_[i] == recipients_[j]) revert DuplicateRecipient();
            }
    ...
    /// @notice Anyone can settle this router's accrued share from a launch reward vault.
    /// @dev The source vault sends ETH back through receive(), which performs exact accounting.
    function collect(address launchVault) external {
        ...
        _collecting = true;
        ILaunchRewardSource(launchVault).claim();
        _collecting = false;
