...
- `packages/agent-core` owns pure agent schemas, policy validation, state transitions and deterministic scoring. `apps/agent-engine` owns the separate paper-only evaluation runtime. Never hide agent execution, signing or treasury behavior inside `apps/market-indexer`.
