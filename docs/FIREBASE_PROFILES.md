...
Production sets `NEXT_PUBLIC_FIREBASE_AUTH_DOMAIN=www.rmtlaunch.fun`. RMT keeps that registered Firebase `authDomain` on production, temporary Vercel previews, and local QA. Preview hostnames may be authorized Firebase callers, but must not become OAuth helper domains: Google redirect URIs are exact-match values and a per-deployment helper would require a new callback registration after every preview. The Vercel rewrite in `apps/web/vercel.json` transparently proxies only `/__/auth/*` to the project's Firebase Hosting origin on the canonical RMT domain.
...
Keep `www.rmtlaunch.fun` in Firebase Authentication's authorized domains and keep the exact `https://www.rmtlaunch.fun/__/auth/handler` URL registered on the Firebase-generated Google OAuth client. Authorize a preview hostname only as a caller when testing it; do not register it as a new auth helper or OAuth callback. Do not use a redirect response in place of the rewrite: the canonical auth helper must be reverse-proxied without changing the browser URL. The original `robinhood-meme-terminal.firebaseapp.com` domain remains the upstream and rollback path.
