...
3. Run the fail-closed live preflight. It must confirm bytecode at the legacy V5 factory, exact official legacy RMT token, PoolManager, and CREATE2 deployer; V5 reports the official RMT name and ticker as protected; and the exact legacy token `0xaB374D24aFBD943a134AdB381D9646e71C6f6C0C` reports RMTMain, `Robinhood Meme Terminal`, and `RMT`. The old governance and registry are not V6 dependencies.
4. Use only the recovery-aware operator console to deploy the new V6 governance/treasury, a fresh 48-hour version registry governed by it and initialized to the V5 factory/version, hook, 0.5% adapter, paused launch gate, market implementation, component-locked policy registry, and V6 factory. The new governance must have the exact one-wallet, 24-hour delay, seven-day window, epoch-1, zero-proposal configuration. The fresh registry, gate, policy registry, canonical protocol treasury, factory creator-payout authority, and every launch splitter must use it. This phase deploys and binds only; it creates zero governance proposals. `DeployMainnetV6OfficialMigration.s.sol` is restricted to the exact mainnet-fork rehearsal and rejects a production run.
5. Run the fail-closed V6 source-verification gate below. It verifies bytecode, hook flags, adapter/factory bindings, the single V6 governance/treasury boundary, fresh registry, guardian, legacy identity factory, exact official legacy token provenance, fees, reserves, delays, official-migration authority, and paused state before submitting the eleven V6 contracts plus the V5 identity factory to Blockscout. The existing legacy token is checked as an immutable dependency, not counted as newly deployed V6 source.
6. Submit Fair, Open, and fresh-registry factory-activation proposals through the same V6 governance. Proposal IDs must come from confirmed `Proposed` events, never a guessed counter.
...
11. Deploy the production-site cutover configuration described below. Confirm the site reads the fresh registry, resolves only its active V6 factory, starts V6 event scans at the exact factory deployment block, and still blocks ordinary creation.
12. From the verified operator wallet, use the exact one-time paused official migration to launch a new `Robinhood Meme Terminal` / `RMT` token contract under the Fair policy. Before signing, acknowledge that it has a new address and new one-billion-token supply and does not copy, swap, credit, or migrate old V5 balances. Confirm the public gate remains paused and verify the launch record, token supply, splitter, market, adapter, and policy.
...
Every browser phase is resumable. A rejected wallet request leaves completed steps recorded, and recovery adopts only exact reviewed onchain events for permissionlessly finalized governance, policy, and registry steps. No browser phase requests a private key or recovery phrase. The console does not trust its recovery file or local-storage source marker at a governance boundary: it revalidates confirmed receipts, exact bindings, all twelve current Blockscout records, and the complete V6 governance proposal-ID set before submitting proposals. Every proposal, including fresh-registry activation, is checked through the public transaction getter for its epoch, cancellation, expiry, approvals, and execution status, and every ID from zero through `transactionCount() - 1` must be proven by an exact saved `Proposed` receipt. The final reopening action repeats the binding, policy, official-migration, delay, governance-history, source, and live production-health checks immediately before the operator-authorized unpause transaction; the launch gate itself rejects outsider execution.
...
Run this immediately after the eight V6 addresses have confirmed deployment receipts and before submitting any delayed governance proposal:
...
The script requires all eight operator-recorded addresses, derives the token implementation, fee-splitter implementation, and official-identity migration from the factory, and verifies twelve contracts: the eleven V6 contracts plus the V5 identity factory. It validates the V6 governance/treasury signer, threshold, delay, execution window, epoch, and zero initial proposal count, then verifies that the fresh registry is owned by it, has the reviewed delay, starts at V5, and has no pending factory. The policy-registry verification input encodes the exact governance treasury, market, and adapter addresses in its constructor arguments and checks the live immutable getters before source submission. The script uses the canonical Robinhood mainnet RPC and Blockscout endpoint unless `ROBINHOOD_MAINNET_RPC_URL` or `BLOCKSCOUT_VERIFIER_URL` is explicitly supplied. It never broadcasts a blockchain transaction and accepts no private key, mnemonic, activation, or unpause input; it does submit source-verification requests to Blockscout. Any chain, code, binding, getter, governance-history, or pre-activation state mismatch stops the run. After it succeeds, return to the operator console: its source-verification phase checks each address through Blockscout's live smart-contract API and requires the expected contract name and compilation target, full—not partial—verification, unchanged bytecode, exact Solidity `v0.8.26+commit.8a97fa7a`, via-IR, and the reviewed optimizer settings. Proposal submission repeats that live check.

For a button-driven run, open the repository's **Verify V6 mainnet sources** GitHub workflow on the frozen release commit and enter the same eight addresses. The workflow runs this exact script without signing material and archives the commit SHA and full log. A workflow run from another commit is not valid release evidence.
