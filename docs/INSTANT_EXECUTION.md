...
Prompt issuance is serialized per wallet across same-origin browser contexts. The lock owner rechecks the durable journals and writes `PROMPT_REQUESTED` before invoking the provider; another context fails closed. Owner rejection is accepted only from the provider's typed rejection signal or EIP-1193 code `4001`, never from message text. After a reload loses the provider promise, recovery may discover a transaction only by exact chain, wallet, recorded nonce, target, value and calldata hash inside the bounded request-block window. A known hash remains blocking unless its exact execution record is terminal.

Exact receipt reconciliation terminalizes the matching known-hash wallet request in the same combined journal write as the execution: `RECEIPT_CONFIRMED` for a successful receipt or `RECEIPT_REVERTED` for a reverted receipt. These terminal request records never block, retain their exact request, plan, payload and transaction-hash audit linkage for the bounded seven-day history window, and age out with the matching execution history. A missing, malformed or still-submitted execution remains fail-closed while its request is `HASH_RECEIVED`.
