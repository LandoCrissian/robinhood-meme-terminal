...
When every blocking passport check is resolved and a current verified metadata receipt exists, the assigned creator can prepare a deterministic private release-review snapshot. Schema-v2 snapshots bind the complete rights revision, exact metadata receipt, accepted consent manifest, proposed payout manifest, and preparation-only economics policy into an immutable server-created record. Legacy schema-v1 snapshots remain readable. Every record remains `prepared`, `simulation_only`, and `contract execution disabled`; it is not approval or publication. See `docs/CREATOR_RELEASE_REVIEW.md`.
