...
export const OUTPUT_LIMIT = 32_768;
const END_RESERVE = 4096;
const schema = "RMT_BASIC_ROUTE_V1";
...
export class ProofJsonl {
  ...
  beginCase(id: string) {
    if (this.active || this.ended || !["USDG_TO_CANNACAT"].includes(id)) throw new Error("PROOF_CASE_INVALID");
  ...
  finish(status: "COMPLETE" | "BLOCKED" | "TIMEOUT" | "FAILED" | "NOT_RUN") {
    ...
    this.emit("PROOF_END", { status: status === "COMPLETE" && (this.cases !== 1 || this.truncated) ? "INCOMPLETE" : status,
      completedCases: this.cases, expectedCases: 1, truncated: this.truncated, recordCount: this.sequence + 1,
  ...
  try {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (record.type === "CASE_START") {
        if (active || fingerprints === 0 || seen.has(record.caseId) || !["USDG_TO_CANNACAT"].includes(record.caseId)) return false;
