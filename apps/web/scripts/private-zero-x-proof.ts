...
import { inspectActions, addressRole, focusedBasicAction } from "./private-zero-x-actions";
...
export function emitObservedCase(writer: ProofJsonl, record: Awaited<ReturnType<typeof observePrivateZeroXCase>>) {
  ...
  if (record.actions) {
    for (const action of record.actions.records) { if (!writer.emit("ACTION", focusedBasicAction(action))) break; }
...
export async function runPrivateZeroXProof(writer = new ProofJsonl(() => {})) {
  ...
  const report = { schema: "RMT_BASIC_ROUTE_PROOF_V1", evidenceClass: "NOT_TESTED",
  ...
  for (const path of ["scripts/private-zero-x-proof.ts", "scripts/private-zero-x-actions.ts", "scripts/private-zero-x-jsonl.ts", "lib/server/vnext-zero-x-adapter.ts",
    ...
    "lib/server/vnext-zero-x-execution-decoder.ts", "lib/server/vnext-zero-x-packed-route.ts", "lib/server/vnext-provider-adapter.ts", "lib/vnext/zero-x-settlement.ts",
