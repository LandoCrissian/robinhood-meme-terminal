  ...
  try {
    ...
    globalThis.fetch = async (raw, init) => {
      ...
      if (url.origin === "https://api.0x.org") {
        ...
        if (firm && mode === "unsupported_basic") body.transaction.data = mutateZeroXActions(body.transaction.data as Hex, a => {
          a.splice(3, 0, encodeFunctionData({ abi: parseAbi(["function BASIC(address sellToken,uint256 proportion,address target,uint256 offset,bytes data)"]),
            functionName: "BASIC", args: [token, 1000000n, other, 36n, `0xdeadbeef${"00".repeat(64)}`] }));
        });
    ...
    for (const [nextMode, expected] of [["allowance", "INSUFFICIENT_ALLOWANCE"], ["balance", "INSUFFICIENT_BALANCE"],
      ...
      ["unsupported", "EXECUTION_ENVELOPE_REJECTED"], ["unsupported_basic", "EXECUTION_ENVELOPE_REJECTED"], ["early_envelope", "EXECUTION_ENVELOPE_REJECTED"], ["rate_limit", "RATE_LIMITED"], ["provider_error", "PROVIDER_UNAVAILABLE"],
      ...
      if (mode === "unsupported" || mode === "early_envelope" || mode === "unsupported_basic") {
        assert.ok((result.actions?.actionCount ?? 0) >= 3);
        assert.equal(result.envelopeDiagnostic?.envelopeReason, mode === "unsupported" ? "UNSUPPORTED_ACTION" : mode === "unsupported_basic" ? "UNSUPPORTED_ROUTE" : "EARLY_SLIPPAGE");
        assert.equal(result.envelopeDiagnostic?.actionIndex, mode === "unsupported_basic" ? 3 : 2);
        assert.equal(result.envelopeDiagnostic?.envelopeFunction, mode === "early_envelope" ? "decodeZeroXExecutableMinimum" : "verifyZeroXEncodedFee");
        const lines: string[] = []; const writer = new ProofJsonl(line => lines.push(line));
        writer.emit("PROOF_START"); writer.emit("SOURCE_FINGERPRINT", { path: "fixture", sha256: "a".repeat(64) });
        writer.beginCase("USDG_TO_CANNACAT"); emitObservedCase(writer, result); writer.finish("COMPLETE");
        assert.ok(validateProofJsonl(lines.join("")));
        ...
        if (mode === "unsupported_basic") {
          const action = emitted.find(r => r.type === "ACTION" && r.index === 3);
          assert.equal(action.callSelector, "0xdeadbeef");
          assert.equal(action.patchOffset, "36");
          assert.equal(action.valueBehavior, "ZERO_CALL_VALUE");
          assert.equal(action.semanticsComplete, false);
          assert.ok(emitted.filter(r => r.type === "ACTION" && r.index !== 3).every(r => r.callSelector === undefined));
        }
      }
