...
async function run() {
  try {
    ...
    for (const pair of [[token, other], [token, zeroAddress], [zeroAddress, token]] as const) {
      const result = await observePrivateZeroXCase(request(...pair));
    ...
    for (const [nextMode, expected] of [["allowance", "INSUFFICIENT_ALLOWANCE"], ["balance", "INSUFFICIENT_BALANCE"],
      ...
      const result = await observePrivateZeroXCase(request());
    ...
    assert.ok(priceCalls > quoteCalls);
    ...
    assert.equal(JSON.parse(child.stdout).evidenceClass, "NOT_TESTED");
