...
import { emitObservedCase } from "./private-zero-x-proof";
...
const other = getAddress("0x0Bd7D308f8E1639FAb988df18A8011f41EAcAD73");
  ...
  try {
    ...
    globalThis.fetch = async (raw, init) => {
      ...
      if (url.origin === "https://api.0x.org") {
        ...
        const body = { liquidityAvailable: true, sellToken, buyToken: url.searchParams.get("buyToken")!, sellAmount: "1000000",
          buyAmount: "1000000", minBuyAmount: "990100", totalNetworkFee: "100000", allowanceTarget: native ? null : holder,
          fees: { integratorFee: { token: sellToken, amount: mode === "wrong_fee" && firm ? "2501" : "2500", type: "volume" }, zeroExFee: null, gasFee: null },
      ...
      if (payload.method === "eth_chainId") result = mode === "wrong_chain" ? "0x1" : "0x1237";
        ...
        ? holderCode : mode === "runtime" ? "0x60016001" : fixture.runtime;
    ...
    const inspected = await verifyZeroXSwapFirmQuote(directRequest, undefined, envelope => {
      ...
      assert.equal(Reflect.set(envelope, "settler", other), false);
      assert.equal(envelope.settler, getAddress(fixture.settler));
    });
    ...
    for (const pair of [[token, other], [token, zeroAddress], [zeroAddress, token]] as const) {
      ...
      assert.equal(result.feeEvidence, "VALIDATED_FIRM_QUOTE_NOT_SETTLEMENT");
      ...
      assert.equal(result.actions?.actionCount, 1);
      ...
      writer.beginCase("ETH_TO_ERC20"); emitObservedCase(writer, result); writer.finish("COMPLETE");
    ...
    for (const [nextMode, expected] of [["allowance", "INSUFFICIENT_ALLOWANCE"], ["balance", "INSUFFICIENT_BALANCE"],
      ...
      ["wrong_fee", "FIRM_QUOTE_REJECTED"], ["rate_limit", "RATE_LIMITED"], ["provider_error", "PROVIDER_UNAVAILABLE"],
      ...
      if (mode === "runtime") assert.equal(result.actions?.actionCount, 1, "capture survives runtime rejection without admission");
