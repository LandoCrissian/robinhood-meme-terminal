...
async function main() {
  ...
  for (let observation = 1; observation <= 2; observation++) for (const input of originals) {
    ...
    for (const amount of amounts) {
      ...
      // Arithmetic comparison only; half-up is no longer execution authority.
  ...
  const report = { observedAt: new Date().toISOString(), chainId: 4663, requestFeeBps: 25, requestSlippagePpm: 9900,
    feeAdmissionChanged: false, atomicPolicy: "REVIEWED_PROVIDER_NATIVE_PERCENTAGE", officialDocumentation: "https://docs.0x.org/evm/0x-swap-api/guides/monetize-your-app-using-swap",
