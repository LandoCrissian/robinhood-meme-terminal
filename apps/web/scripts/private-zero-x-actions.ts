  ...
  for (const [index, action] of actions.slice(0, MAX_ACTIONS).entries()) {
    ...
    try {
      ...
      if (decoded.functionName === "BASIC") {
        ...
        r.callSelector = /^0x[0-9a-f]{8}/i.test(nested) ? nested.slice(0, 10).toLowerCase() : null;
        r.targetRuntimeFixed = false; // BASIC target comes from calldata, not the runtime.
        r.outputTokenRole = "UNKNOWN_TARGET_SEMANTICS";
        r.outputDestination = "UNKNOWN_TARGET_SEMANTICS";
        r.nestedBytes = (nested.length - 2) / 2;
        const noPatch = same(token, zeroAddress), native = same(token, ZERO_X_NATIVE_TOKEN);
        r.patchInBounds = noPatch || native && nested === "0x" ? offset === 0n : offset + 32n <= BigInt(r.nestedBytes);
        r.selectorPreservedByPatch = noPatch || offset >= 4n;
        r.valueBehavior = native ? "PROPORTIONAL_NATIVE_VALUE" : "ZERO_CALL_VALUE";
        r.approvalBehavior = !noPatch && !native && !same(token, target) ? "SETTLER_TOKEN_APPROVAL_TO_TARGET" : "NO_BASIC_APPROVAL";
        r.callAuthority = "SETTLER_CALLER_TARGET_BEHAVIOR_UNPROVEN";
        ...
        if (native && nested === "0x" && offset === 0n) {
          ...
          try {
            ...
            if (inner.functionName === "transfer" && nested.length === 138 && (noPatch && offset === 0n || same(token, target) && offset === 36n)) {
              ...
              r.outputTokenRole = addressRole(target, roles); r.outputDestination = addressRole(inner.args[0], roles);
...
/** Narrow output projection: only BASIC at the reported position gets details.
 * Other actions retain order/kind only. This does not authorize any call.
 */
export function focusedBasicAction(record: StructuralAction): StructuralAction {
  const result: StructuralAction = { index: record.index, actionKind: record.actionKind,
    selector: record.selector, semanticsComplete: false };
  if (record.index !== 3 || record.actionKind !== "BASIC") return result;
  for (const key of ["semanticFamily", "sourceSemantics", "tokenRole", "recipientRole", "targetRole",
    "callSelector", "nestedKind", "nestedBytes", "patchOffset", "patchInBounds", "selectorPreservedByPatch",
    "amountMode", "numerator", "denominator", "amountLiteral", "literalOverwritten", "rounding", "balanceBasis",
    "valueBehavior", "approvalBehavior", "callAuthority", "targetRuntimeFixed", "outputTokenRole", "outputDestination",
    "affectsSellBalance", "affectsBuyBalance", "semanticsComplete"] as const) {
