...
export default function PrivacyPage() {
  return (
    <LegalShell eyebrow="DATA TRANSPARENCY" title="Privacy Notice" updated="July 29, 2026">
      ...
      <p>Creator consent invitations and final response receipts are stored privately for the assigned creator and RMT administrators. A shareable consent link contains creator-supplied collaborator name, wallet, role, proposed share, project, asset revision, terms fingerprint, and expiration in the link fragment. Anyone who receives or copies the link can read those fields. A separate public marker exposes only the invitation fingerprint, project and asset identifiers, expiration, and pending, revoked, accepted, or rejected status so the signing page can fail closed on unusable links. Signed response receipts contain the wallet address, action, signed response time, server receipt time, invitation fingerprint, and public wallet signature.</p>
      <p>Creator release-review snapshots are immutable private preparation records accessible to the assigned creator and RMT administrators. They copy the selected rights draft, accepted consent evidence, proposed payout wallets, and simulation economics policy into a deterministic fingerprint. Preparing a snapshot does not publish the information, approve the release, or enable a blockchain transaction.</p>
      <p>RMT administrators may attach one immutable private preparation decision containing a structured outcome, reason, review note, reviewer identifier, and decision fingerprint. The assigned creator can read that decision. It does not enable minting, listing, fees, payouts, or contract deployment.</p>
