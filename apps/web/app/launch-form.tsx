...
export function LaunchForm() {
  ...
  return (
    <section className="panel launchConfigPanel">
      ...
      {lockOfficialFields && !officialPausedLaunch && <div className="callout mainnetWarning"><strong>Historical V6 launch workflow is prefilled</strong><span>This retired launchpad workflow is preserved for historical evidence and is not a current RMT product or release path.</span></div>}
      ...
      {deployed && <div className="launchSuccess"><strong>Launch #{deployed.launchId.toString()} confirmed</strong><span>Token, market, and fee splitter were created in one transaction.</span><Link href={`/project/${deployed.token}?launch=${deployed.launchId.toString()}`}>Open project page →</Link></div>}
