...
export function RewardVaultPanel({ tokenAddress, symbol }: { tokenAddress: Address; symbol: string }) {
  ...
  return (
    <section className="panel rewardDashboard">
      ...
      <div className="callout feeAuthorityNote"><strong>{recipientRedirected ? "Future collected creator-share fees currently route to RMT" : "Creator payout control is narrowly governed"}</strong><span>The token creator cannot initiate, accept, or execute a payout change. Only delayed RMT governance can move future creator-share collections between the immutable original creator and RMT treasury, using a nonzero public evidence hash and the current replay-protection nonce. RMT treasury can invalidate a stale unexecuted nonce but cannot choose another recipient. Previously paid or deferred ETH and token fees remain with the wallet that earned them. Uncollected pool fees use the recipient active when collection occurs, even if some accrued earlier.</span></div>
