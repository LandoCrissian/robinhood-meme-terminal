...
export function PositionGuardPanel({
  ...
  onPrepareExit: (amount: string) => void;
}) {
  ...
  useEffect(() => {
    ...
    const next = advancePositionGuard(guard, currentValueUsd);
    if (
  ...
  }, [currentValueUsd, guard]);

  const evaluation = useMemo(
    () => guard ? evaluatePositionGuard(guard, currentValueUsd) : null,
    [currentValueUsd, guard]
  );
  ...
  function quickProtect() {
    ...
    setGuard(next);
    setEditing(false);
    setMessage("Position Guard armed with a 20% trailing floor.");
  }

  function prepareExit(exitBps: number) {
    const amount = tokenAmountForExit(balance, exitBps);
    if (!amount) {
      setMessage("RMT could not calculate an exit amount from this balance.");
      return;
    }
    onPrepareExit(amount);
  }

  function resetTrigger() {
    if (!guard) return;
    const next = resetPositionGuardTrigger(guard, currentValueUsd);
  ...
  function dismissPrincipalTarget() {
    if (!guard) return;
    const next = acknowledgePrincipalRecovery(guard);
    if (writePositionGuard(next)) setGuard(next);
  }
      ...
      <section className="positionGuardPanel idle" aria-labelledby="position-guard-heading">
        ...
        {!editing ? (
          <div className="positionGuardStart">
            <button type="button" onClick={quickProtect}>Quick protect · 20% trail</button>
      ...
      {evaluation && (
        <>
          <div className="positionGuardMetrics">
            ...
            <span><small>RECORDED PEAK</small><strong>{money(evaluation.highWatermarkUsd)}</strong><i>On this device</i></span>
          ...
          {evaluation.stopTriggered && (
            <div className="positionGuardTrigger" role="alert">
              ...
              <button type="button" onClick={() => prepareExit(10_000)}>Prepare full exit</button>
              <button type="button" onClick={resetTrigger}>Keep position · reset trail</button>
            </div>
          )}
          {evaluation.principalRecoveryReady && !evaluation.stopTriggered && (
            <div className="positionGuardProfit" role="alert">
              <span><small>2× TARGET REACHED</small><strong>Recover the original {money(guard.basisUsd)}</strong><em>Estimated {percent(evaluation.principalRecoveryBps)} of holdings</em></span>
              <button type="button" onClick={() => prepareExit(evaluation.principalRecoveryBps)}>Prepare principal exit</button>
              <button type="button" onClick={dismissPrincipalTarget}>Mark handled</button>
            </div>
          )}
        </>
      )}
