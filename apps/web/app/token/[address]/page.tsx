  ...
  return (
    <main className="detailPage">
      ...
      <section className="tokenHero panel">
        ...
        <div><p className="eyebrow">VERIFIED ONCHAIN TOKEN</p><h1>{nameRead.data}</h1><p className="tokenSymbol">${symbolRead.data}</p>{metadata?.description && <p className="tokenDescription">{metadata.description}</p>}{metadata && (metadata.website || metadata.x || metadata.telegram) && <div className="socialLinks">{metadata.website && <a href={metadata.website} target="_blank" rel="noopener noreferrer">Website ↗</a>}{metadata.x && <a href={metadata.x} target="_blank" rel="noopener noreferrer">X ↗</a>}{metadata.telegram && <a href={metadata.telegram} target="_blank" rel="noopener noreferrer">Telegram ↗</a>}</div>}<WatchlistButton address={tokenAddress} name={nameRead.data} symbol={symbolRead.data} image={metadata?.image} /><TokenShareActions address={tokenAddress} name={nameRead.data} symbol={symbolRead.data} /></div>
      </section>
