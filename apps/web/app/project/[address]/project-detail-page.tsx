  ...
  return (
    <main className="detailPage">
      ...
      <section className="tokenHero panel">
        ...
        <div className="tokenHeroCopy"><div className="tokenOriginLine"><p className="eyebrow">RMT V6 · HISTORICAL PROJECT EVIDENCE</p><span>Recorded on {activeNetworkLabel}</span></div><h1>{nameRead.data}</h1><p className="tokenSymbol">${symbolRead.data}</p>{metadata?.description && <p className="tokenDescription">{metadata.description}</p>}<div className="tokenHeroActions"><WatchlistButton address={tokenAddress} name={nameRead.data} symbol={symbolRead.data} image={metadata?.image} launchId={launch.launchId.toString()} compactLabel /><TokenShareActions address={tokenAddress} name={nameRead.data} symbol={symbolRead.data} launchId={launch.launchId.toString()} /></div></div>
      </section>
