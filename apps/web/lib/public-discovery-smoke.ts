...
const legacyHomeSource = readFileSync(new URL("../app/legacy-terminal-page.tsx", import.meta.url), "utf8");
...
assert.match(legacyHomeSource, /<OfficialRmtMarket \/>/);
assert.doesNotMatch(legacyHomeSource, /<FreshLaunchFeed \/>/);
...
assert.doesNotMatch(externalMarketFeedSource, /api\/trade\/external-availability/);
assert.match(externalMarketFeedSource, /Execution route checked on open/);
assert.match(externalMarketFeedSource, /href=\{`\/market\/\$\{market\.address\}\?side=buy`\}/);
assert.match(externalMarketFeedSource, /href=\{`\/market\/\$\{market\.address\}\?side=sell`\}/);
assert.doesNotMatch(externalMarketFeedSource, /Launch sources/);
assert.match(externalMarketFeedSource, /Project provenance is attached only/);
