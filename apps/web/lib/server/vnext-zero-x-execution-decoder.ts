  ...
  try {
    ...
    if (native) {
      ...
      if (wrap.functionName !== "BASIC" || getAddress(wrap.args[0]) !== ZERO_X_NATIVE_TOKEN || wrap.args[1] !== basis
        ...
        || wrap.args[4].toLowerCase() !== toFunctionSelector("deposit()") + "0".repeat(64)) fail("NATIVE_WRAP_MISMATCH");
      routeInput = WETH;
    }
    const route = next();
    ...
    if (route.functionName === "UNISWAPV3") {
      ...
      if (getAddress(`0x${path.slice(2, 42)}`) !== routeInput) fail("ROUTE_INPUT_MISMATCH");
      ...
      if (getAddress(recipient) !== envelope.settlerTarget) fail("ROUTE_RECIPIENT_MISMATCH");
      if (fromZeroXToken(token) !== routeInput) fail("ROUTE_INPUT_MISMATCH");
      if (proportion !== basis) fail("ROUTE_RATE_MISMATCH");
      ...
      while (cursor < fills.length) {
        ...
        if (Number.parseInt(take(1), 16) > 1) fail("UNSUPPORTED_POOL_MANAGER");
        take(3); take(32);
    ...
    if (input.outputAsset === zeroAddress && routeOutput === WETH) {
      ...
      if (unwrap.functionName !== "BASIC" || getAddress(unwrap.args[0]) !== WETH || unwrap.args[1] !== basis
        ...
        || unwrap.args[4].toLowerCase() !== toFunctionSelector("withdraw(uint256)") + "0".repeat(64)) fail("NATIVE_UNWRAP_MISMATCH");
