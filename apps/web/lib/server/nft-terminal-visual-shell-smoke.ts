...
assert.match(catalog, /Robinhood Chain NFT Markets/);
assert.match(catalog, /Active[\s\S]*New[\s\S]*Minting[\s\S]*Trending[\s\S]*Watching/);
assert.match(catalog, /catalog\.projects\.length/);
assert.match(catalog, /Search collection, contract or NFT/);
assert.match(catalog, /data-nft-collection-row/);
assert.match(catalog, /data-rmt-registration-frame/);
assert.match(catalog, /data-nft-mint-radar/);
assert.match(catalog, /No verified live mints/);
assert.match(catalog, /Discovery only · not RMT admission/);
assert.match(catalogStyles, /\.collectionRow\s*\{[\s\S]*grid-template-columns:/);
assert.match(catalogStyles, /@media\s*\(max-width:\s*760px\)[\s\S]*\.collectionRow\s*\{/);
assert.match(catalogStyles, /\.collectionMetrics\s*\{/);
...
assert.match(project, /FLOOR/);
assert.match(project, /OPENSEA REPORTED SALE/);
assert.match(project, /market meaning not established/);
assert.match(project, /data-nft-gallery/);
assert.match(project, /data-nft-evidence-ledger/);
assert.match(project, /Collection market views/);
assert.match(projectStyles, /grid-template-columns:\s*repeat\(5/);
assert.match(projectStyles, /\.collectionSpotlight/);
assert.match(inventoryStyles, /grid-template-columns:\s*repeat\(6/);
assert.match(inventoryStyles, /@media\s*\(max-width:\s*700px\)[\s\S]*grid-template-columns:\s*repeat\(2/);
...
assert.doesNotMatch([catalog, project, item].join("\n"), />\s*(BUY|LIST|OFFER|FULFILL|SIGN|SUBMIT|ACCEPT|SWEEP)\s*</i);
