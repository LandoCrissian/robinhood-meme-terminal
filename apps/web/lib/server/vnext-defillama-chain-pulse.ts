...
function toPulseStatus(tvlStatus: SourceStatus, dexStatus: SourceStatus, feesStatus: SourceStatus): PulseStatus {
  const readyCount = [tvlStatus, dexStatus, feesStatus].filter((status) => status === "ready").length;
  if (readyCount === 3) {
    return "ready";
  }
  if (readyCount > 0) {
...
function normalizeFeesComponents(result: {
  components?: {
    dailyFees: { status: SourceStatus; reason?: string; total24hUsd?: number; total7dUsd?: number };
    dailyRevenue: { status: SourceStatus; reason?: string; total24hUsd?: number; total7dUsd?: number };
...
export async function readVNextDefiLlamaChainPulse(
  ...
  const status = toPulseStatus(tvlStatus, dexStatus, feesStatus);
  ...
  return {
    ...
    fees24hUsd:
      feesStatus === "ready"
        ...
        : null,
    fees7dUsd: feesStatus === "ready" ? toNullableNumber((fees as { fees7dUsd?: number }).fees7dUsd) : null,
    revenue24hUsd:
      feesStatus === "ready"
        ? toNullableNumber((fees as { revenue24hUsd?: number }).revenue24hUsd)
        : null,
    revenue7dUsd: feesStatus === "ready" ? toNullableNumber((fees as { revenue7dUsd?: number }).revenue7dUsd) : null,
    ...
    sources: {
      ...
      fees: {
        ...
        reason: feesReason,
        components: normalizeFeesComponents(fees)
      }
    }
  };
}
