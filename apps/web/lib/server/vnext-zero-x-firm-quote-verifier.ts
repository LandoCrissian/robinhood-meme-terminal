...
export type ZeroXExecutableInspection = Readonly<{ target: Address; data: Hex; settler: Address }>;
...
export async function verifyZeroXSwapFirmQuote(request: VNextProviderVerificationRequest, observe?: ZeroXFirmObserver,
  inspect?: (envelope: ZeroXExecutableInspection) => void): Promise<ZeroXSwapFirmQuoteVerificationEvidence> {
  ...
  inspect?.(Object.freeze({ target: quote.transactionTarget, data: quote.calldata, settler: quote.settlerTarget }));
  observe?.({ kind: "quote", allowanceTarget: request.inputAsset === zeroAddress ? null : configuration.allowanceHolder, transactionTarget: quote.transactionTarget,
    transactionValue: quote.transactionValueAtomic, calldataHash: keccak256(quote.calldata), decodedSettler: quote.settlerTarget,
