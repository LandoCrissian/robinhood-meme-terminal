...
type CurrentLaunchpadSettledSources = {
  stonk: PromiseSettledResult<ExternalMarket[]>;
  ponsV1: PromiseSettledResult<ExternalMarket[]>;
  ponsV2: PromiseSettledResult<ExternalMarket[]>;
  lemon: PromiseSettledResult<ExternalMarket[]>;
  sushi: PromiseSettledResult<SushiLaunchSnapshot>;
};

export function assembleCurrentLaunchpadSnapshot({
  stonk,
  ponsV1,
  ponsV2,
  lemon,
  sushi
}: CurrentLaunchpadSettledSources): CurrentLaunchpadSnapshot {
...
export async function fetchCurrentLaunchpadSnapshot(
  ...
  const [stonk, ponsV1, ponsV2, lemon, sushi] = await Promise.allSettled(tasks);
  return assembleCurrentLaunchpadSnapshot({ stonk, ponsV1, ponsV2, lemon, sushi });
}
