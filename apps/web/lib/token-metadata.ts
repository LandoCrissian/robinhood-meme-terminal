...
function safeMetadata(value: unknown): TokenMetadata | null {
  ...
  return { name: text("name")?.slice(0, 80), symbol: text("symbol")?.slice(0, 20), description: text("description")?.slice(0, 1_000), image: image && (image.startsWith("ipfs://") || image.startsWith("https://")) ? image : undefined };
}
