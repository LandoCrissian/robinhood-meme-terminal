...
export const DISTRIBUTION_PLANNER_NOT_APPROVED_LABEL = "NOT APPROVED";
...
function emptyState(actionKind: DistributionPlannerActionKind, message: string): DistributionPlannerErrorState {
  return {
    status: "error",
    actionKind,
    message,
...
export function buildDistributionPlannerPreview(input: DistributionPlannerInput): DistributionPlannerState {
  ...
  if (!csv.trim()) {
    return emptyState(actionKind, "Paste CSV rows to begin planning.");
  }
