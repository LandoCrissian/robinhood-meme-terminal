...
const legacyStyles = readFileSync(new URL("../../app/vnext/vnext-terminal.css", import.meta.url), "utf8");
const reconstructionStyles = readFileSync(new URL("../../app/vnext/vnext-terminal-reconstruction.css", import.meta.url), "utf8");
const styles = `${legacyStyles}\n${reconstructionStyles}`;
...
assert.match(rootPage, /import "\.\/vnext\/vnext-terminal-reconstruction\.css"/);
...
assert.match(presentationBoundary, /\(min-width: 761px\)/);
...
assert.match(presentations, /className="rmtMobileHeader"[\s\S]*<VNextWalletConnection showFunding=\{false\} \/>/);
assert.match(walletConnection, /showFunding = true/);
assert.match(walletConnection, /<WalletButton target="mainnet" returnTo="\/" showFunding=\{showFunding\} \/>/);
assert.match(shell, /href="\/" aria-label="RMT Terminal home"/);
assert.match(shell, /Robinhood Terminal/);
...
assert.match(presentations, /className="rmtMobileDiscovery"/);
assert.match(presentations, /aria-label="Market categories"/);
assert.match(presentations, /viewCounts\[candidate\.id\]/);
...
assert.match(reconstructionStyles, /\.rmtDesktopWorkstation[\s\S]*grid-template-columns: 254px minmax\(470px, 1fr\) 368px/);
assert.match(reconstructionStyles, /\.rmtMobileTradeDock[\s\S]*grid-template-columns: 1fr 1fr/);
