...
assert.match(composer, /failed: \\(cause\\) =>[\\s\\S]*const phase = failureJourneyPhase\\(cause,[\\s\\S]*const message = phase === "QUOTE_EXPIRED" \\? "Price changed\\. Retry quote\\."[\\s\\S]*setQuoteState\\(\\{ state: "error", message, phase \\}\\)[\\s\\S]*setVerificationState\\(\\{ state: "error", message \\}\\)[\\s\\S]*setAuthorizationState\\(\\{ state: "error", message \\}\\)/);
