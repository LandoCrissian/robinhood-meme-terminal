...
const capitalFlow = readFileSync(new URL("../../app/vnext/vnext-capital-flow-card.tsx", import.meta.url), "utf8");
...
assert.match(workspace, /Contract source/);
...
assert.match(directory, /useVisibilityRefresh\(refreshEcosystemDirectory/);
assert.doesNotMatch(directory, /requestIdleCallback/);
assert.doesNotMatch(directory, /if \(!hasData\.current\) return/);
assert.match(directory, /if \(hasData\.current\) publishMarkets\(\)/);
assert.match(presentations, /Loading market data/);
assert.match(presentations, /Market data pending/);
assert.match(presentations, /Market data delayed/);
assert.match(shellController, /activityCoverageDelayed: enrichmentStatus === "delayed"/);
assert.match(presentations, /candidate\.id === "active" \|\| candidate\.id === "trending" \|\| candidate\.id === "new"/);
assert.match(presentations, /walletReadPending && candidate\.id === "held"/);
assert.match(presentations, /<VNextCapitalFlowCard \/>/);
assert.match(capitalFlow, /Capital Flow/);
assert.match(capitalFlow, /Market context · DeFiLlama/);
...
assert.match(composer, /if \(!authorizationEnabled \|\| stockTokenViewOnly\) return/);
assert.match(composer, /Trading activation pending/);
assert.match(composer, /Preview mode shows informational routes only\. RMT will not connect your wallet or prepare a transaction until verified execution is activated\./);
assert.match(composer, /disabled=\{!authorizationEnabled \|\| stockTokenViewOnly/);
...
assert.match(composer, /Buy confirmed/);
assert.match(composer, /Sell confirmed/);
...
assert.match(composer, /ExplorerLink/);
