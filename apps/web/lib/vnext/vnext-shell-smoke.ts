...
assert.match(composer, /failed: \\(cause\\) => \\{/);
assert.match(composer, /const phase = failureJourneyPhase\\(cause,/);
assert.match(composer, /const message = phase === "QUOTE_EXPIRED" \\? "Price changed\\. Retry quote\\."/);
assert.match(composer, /setQuoteState\\(\\{ state: "error", message, phase \\}\\)/);
assert.match(composer, /setVerificationState\\(\\{ state: "error", message \\}\\)/);
assert.match(composer, /setAuthorizationState\\(\\{ state: "error", message \\}\\)/);
