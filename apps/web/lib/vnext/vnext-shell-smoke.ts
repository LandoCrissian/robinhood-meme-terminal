...
import { capitalFlowStatusLabel, heldCountIsIndeterminate, heldCountLabel } from "./terminal-presentation-state";
...
const primaryAction = readFileSync(new URL("./trade-primary-action.ts", import.meta.url), "utf8");
...
assert.equal(capitalFlowStatusLabel("loading"), "CHECKING");
assert.equal(capitalFlowStatusLabel("error"), "DATA UNAVAILABLE");
assert.equal(capitalFlowStatusLabel("unavailable"), "DATA UNAVAILABLE");
assert.equal(capitalFlowStatusLabel("partial"), "PARTIAL DATA");
assert.equal(capitalFlowStatusLabel("ready"), "LIVE");
assert.equal(heldCountIsIndeterminate("idle"), true);
assert.equal(heldCountIsIndeterminate("loading"), true);
assert.equal(heldCountIsIndeterminate("error"), true);
assert.equal(heldCountIsIndeterminate("ready"), false);
assert.equal(heldCountIsIndeterminate("stale"), false);
assert.equal(heldCountLabel("loading", 0), "…");
assert.equal(heldCountLabel("error", 0), "…");
assert.equal(heldCountLabel("ready", 0), 0);
assert.equal(heldCountLabel("ready", 3), 3);
assert.equal(heldCountLabel("stale", 2), 2);
...
assert.match(presentations, /<SpendBalance[\s\S]*onWalletSnapshotChange=\{props\.onWalletSnapshotChange\}/);
assert.match(shellController, /selectedVNextWalletReadAddress\(\{[\s\S]*selectedWalletKey: identity\.activeWalletKey[\s\S]*connectorUid: account\.connector\?\.uid[\s\S]*walletReadSnapshot\.walletKey === identity\.activeWalletKey/,
  "The shell must synchronously hide a prior account's balance snapshot before passive effects publish the new account.");
...
assert.match(presentations, /function DesktopMarkets[\s\S]*rmtScannerControls[\s\S]*<DirectoryRows[\s\S]*<VNextChainPulseCard/);
assert.match(presentations, /function MobileMarkets[\s\S]*<MarketCategoryNav[\s\S]*<MarketSearch[\s\S]*<DirectoryRows[\s\S]*<VNextChainPulseCard/);
assert.match(presentations, /function DirectoryRows[\s\S]*not classified as Active/);
...
assert.match(workspace, /Observed market activity/);
assert.match(workspace, /aria-label="Exact-pool trade tape"/);
assert.doesNotMatch(workspace, />Confirmed activity</);
...
assert.match(workspaceHook, /readPublicWorkspace<ExternalMarketResponse>\(marketUrl\)/);
assert.match(workspaceHook, /readPublicWorkspace<WorkspaceResolutionResponse>\(coreUrl\)/);
assert.match(workspaceHook, /coreSnapshot && \(!externalMarketLookup \|\| marketSnapshot\) \? "ready" : "partial"/);
...
assert.match(presentations, /candidate\.id === "active" \|\| candidate\.id === "movers" \|\| candidate\.id === "trending" \|\| candidate\.id === "new"/);
assert.match(presentations, /candidate\.id === "held" \? heldCountLabel\(walletReadStatus, counts\.held\)/);
assert.equal((presentations.match(/<MarketCategoryNav[^>]+walletReadStatus=\{props\.walletReadStatus\}/g) ?? []).length, 3);
assert.match(presentations, /<VNextCapitalFlowCard \/>/);
assert.match(capitalFlow, /Capital Flow/);
assert.match(capitalFlow, /status: "loading", flow: undefined/);
assert.match(capitalFlow, /Checking stablecoin liquidity · No values reported yet/);
assert.match(capitalFlow, /capitalFlowStatusLabel\(status\)/);
...
assert.match(composer, /Comparison quotes are estimates only/);
assert.match(composer, /No protected minimum is established before verification/);
assert.match(composer, /\? "Sign in"[\s\S]*\? "Choose trading wallet" : "Wallet options"/);
...
assert.match(composer, /disabled=\{rmtTradePrimaryActionDisabled\(\{[\s\S]*authorizationEnabled,[\s\S]*stockTokenViewOnly,/);
assert.match(primaryAction, /return !input\.authorizationEnabled[\s\S]*\|\| input\.stockTokenViewOnly/);
...
assert.ok(composer.includes("failed: (cause) => {"));
assert.ok(composer.includes("const phase = failureJourneyPhase(cause,"));
assert.ok(composer.includes('const message = phase === "QUOTE_EXPIRED" ? "Price changed. Retry quote."'));
assert.ok(composer.includes('setQuoteState({ state: "error", message, phase })'));
assert.ok(composer.includes('setVerificationState({ state: "error", message })'));
assert.ok(composer.includes('setAuthorizationState({ state: "error", message })'));
assert.match(composer, /currentPreparationContext\.current !== key/);
assert.match(composer, /refreshCoordinator\.current\.run/);
