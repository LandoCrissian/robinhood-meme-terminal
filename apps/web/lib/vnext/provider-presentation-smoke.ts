...
assert.equal(vNextProviderLabel("zero-x-swap"), "0x");
assert.equal(vNextProviderRoutePresentation({ provider: "zero-x-swap", route: "aggregated" }).routeLabel, "Best available Robinhood liquidity");
