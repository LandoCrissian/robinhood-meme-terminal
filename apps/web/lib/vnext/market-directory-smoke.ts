...
assert.match(route, /if \(base === ROBINHOOD_WETH_ADDRESS\.toLowerCase\(\)\) return pair\.baseToken/);
assert.match(route, /if \(quote === ROBINHOOD_WETH_ADDRESS\.toLowerCase\(\)\) return pair\.quoteToken/);
assert.match(route, /basePriceUsd \/ basePriceInQuote/);
assert.match(route, /marketCapUsd: selectedIsBase/);
assert.match(route, /priceChange24h: selectedIsBase/);
